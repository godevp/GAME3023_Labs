...
public class CharacterMovement : MonoBehaviour
{
    ...
    {
        if(collision.gameObject.tag == "musicBox")
        {
            MusicManager.Instance.CrossFadeTo(MusicManager.TrackID.InsideOfCollision, 1.5f);
        }
    }
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "musicBox")
        {
            MusicManager.Instance.CrossFadeTo(MusicManager.TrackID.OutOfCollision, 1.5f);
        }
    }
}
